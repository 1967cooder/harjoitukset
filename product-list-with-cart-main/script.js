...
document.addEventListener("DOMContentLoaded", () => {
  ...
  confirmOrderBtn.style.display = "none";
  carbonText.style.display = "none"; // ✅ скрито по начало
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateCartUI() {
    ...
    if (cart.length === 0) {
      ...
      cartItemsContainer.style.display = "none";
      orderSummary.style.display = "none";
      carbonText.style.display = "none"; // ✅ Скрито, когато е празно
      confirmOrderBtn.style.display = "none";
      cartItemsContainer.innerHTML = "";
      orderSummary.textContent = "Order Total";
    } else {
      ...
      cartItemsContainer.style.display = "block";
      orderSummary.style.display = "block";
      carbonText.style.display = "block"; // ✅ Показва се, когато има продукти
      confirmOrderBtn.style.display = "block";
...
[decreaseBtn, increaseBtn].forEach((btn) => {
  ...
  function updateCartUI() {
    ...
    if (cart.length === 0) {
      ...
      confirmOrderBtn.style.display = "none";
      cartItemsContainer.innerHTML = "";
      orderSummary.querySelector('strong').textContent = "$0.00";
    } else {
      emptyImage.style.display = "none";
      emptyText.style.display = "none";
      cartItemsContainer.style.display = "block";
      ...
      cartItemsContainer.innerHTML = cart
        ...
        <div class="cart-item">
          <div>
            <h4>${item.name}</h4>
            <p><span class="item-quantity">${item.quantity}x</span>$${item.price.toFixed(
            2
          )} 
            <strong>$${(item.price * item.quantity).toFixed(2)}</strong></p>
          </div>
      ...
      orderSummary.querySelector('strong').textContent = `$${total.toFixed(2)}`;
    }
  ...
  function renderConfirmedItems() {
    ...
    orderTotalConfirmed.innerHTML = `<span>Order Total</span><span> <strong>$${total.toFixed(
      2
    )}</strong></span>`;
  }
