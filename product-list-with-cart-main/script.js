...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateCartUI() {
    ...
    if (cart.length === 0) {
      ...
      // ✅ Скриваме съдържанието и бутона
      cartItemsContainer.style.display = "none";
      orderSummary.style.display = "none";
      carbonText.style.display = "none";
    ...
    // ✅ Показваме съдържанието и бутона
    cartItemsContainer.style.display = "block";
    orderSummary.style.display = "block";
    carbonText.style.display = "block";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("data.json")
    ...
    .then((products) => {
      products = products.map((p, index) => ({ id: index + 1, ...p }));
      renderProducts(products);
      updateCartUI(); // ✅ Обновяваме UI веднага (скрива празната количка правилно)
    })
    .catch((error) => console.error("Грешка при зареждане на продуктите:", error));
  ...
  function updateCartUI() {
    ...
    if (cart.length === 0) {
      ...
      orderSummary.style.display = "none";
      carbonText.style.display = "none";
      confirmOrderBtn.style.display = "none"; // ✅ Бутонът изчезва при празна количка
