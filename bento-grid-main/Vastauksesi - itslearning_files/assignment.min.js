...
(function(c){var e=CCL.CommonConstants,d=function(){return function(a,b,f){var c=this;this.urls=a;this.fileId=b;this.signedFileId=f;this.save=function(b){var a=new FormData;a.append("file",b,c.fileId);return new Promise(function(b,f){var d=new XMLHttpRequest;d.open("GET",c.urls.FileRepoUploadTokenUrl+"?FileId="+encodeURIComponent(c.fileId)+"&SignedFileId="+encodeURIComponent(c.signedFileId),!0);d.responseType="json";d.onload=function(){if(d.status===e.httpStatusCodes.ok){var g=d.response,k=new XMLHttpRequest;
k.open("PUT",c.urls.FileRepoUrl+"/"+encodeURIComponent(c.fileId),!0);k.setRequestHeader("Authorization","Bearer "+g);k.onload=function(){k.status===e.httpStatusCodes.ok?b():f("Cannot update pdf file in filerepo.")};k.send(a)}else f("Cannot get filerepo upload token.")};d.send()})}}}();c.FileRepoUpdater=d})(Annotations||(Annotations={}));
...
Promise.resolve();this.fileRepoUpdater=new c.FileRepoUpdater(a.Urls,a.FileRepoId,a.SignedFileRepoId);this.statusMessageSaving=document.getElementById("document-status-saving");this.statusMessageSaved=document.getElementById("document-status-saved");document.addEventListener(c.AnnotationApp.PdfViewerInitializedEventType,function(){c.AnnotationApp.pdfViewerInstance.eventBus.on("documentloaded",b.attachEventListeners)})}d.prototype.toggleStatusMessages=function(a){a=a&&!this.saveInProgress&&null==this.timeoutId;
this.statusMessageSaving.style.display=a?"none":"";this.statusMessageSaved.style.display=a?"":"none"};return d}();c.FillableFormsManager=e})(Annotations||(Annotations={}));
...
d.number)});setTimeout(function(){f.drawingManager.ensureDrawing()},100);this.detectFillableForm().then(function(){f.attachTextLayerEvents(a.source.textLayerDiv,d)})}};d.prototype.detectFillableForm=function(){var a=this;return new Promise(function(b){a.settings.IsFillableFormsEnabled&&null===a.hasFillableForms?a.checkAnnotationsStartingFromPage(1).then(function(){b()}):b()})};d.prototype.checkAnnotationsStartingFromPage=function(a){var b=this;return new Promise(function(f){c.AnnotationApp.pdfViewerInstance.pdfDocument.getPage(a).then(function(d){d.getAnnotations().then(function(d){if(0<
...
e&&e.UserId&&0<e.UserId.length&&$(function(){window[c.Constants.ClientInstanceNames.AllInOneGrader].open(e)})}__extends(a,d);a.prototype.onGridClickableClick=function(b){b=$(b.currentTarget).parents("tr").find("button.h-hidden");0<b.length&&(b=JSON.parse(b.attr(a.assessLinkDataAttribute)),window[c.Constants.ClientInstanceNames.AllInOneGrader].open(b,this.headersList))};a.prototype.updateUi=function(){if(!1===this.currentZipTaskInfo.isValid)this.$feedbackText.html(""),this.stopPolling(),this.$feedbackClose.show();
else switch(this.currentZipTaskInfo.status){case this.settings.data.status.NotStarted:case this.settings.data.status.Waiting:case this.settings.data.status.InProgress:case this.settings.data.status.InProgressExternal:this.$feedbackText.html(this.settings.data.downloadingMessageTemplate);break;case this.settings.data.status.Ok:var a=this.settings.terms.readyMessageTemplate;a=a.replace(this.settings.data.fileRepoToken,encodeURIComponent(this.currentZipTaskInfo.result));this.$feedbackText.html(a);this.stopPolling();
this.$feedbackClose.show();this.$feedback.show();this.$itemDownloadAnswersLink.show();break;case this.settings.data.status.Error:this.stopPolling(),this.$feedback.hide(),this.$itemDownloadAnswersLink.show(),this.$errorFeedbackText.html(this.currentZipTaskInfo.result),this.$errorFeedback.show()}};a.prototype.checkAsyncTaskStatusAndUpdateUi=function(){var a=this;$.ajax({type:"GET",url:this.settings.urls.serviceUrl,dataType:"json"}).done(function(b){a.currentZipTaskInfo.status!==b.Status&&(a.currentZipTaskInfo.status=
b.Status,a.currentZipTaskInfo.result=b.Result,a.updateUi())}).fail(function(){a.stopPolling();a.$feedback.hide();a.$errorFeedbackText.html(a.settings.terms.errorOccured);a.$errorFeedback.show()})};a.prototype.checkStatus=function(){this.currentZipTaskInfo.isValid&&this.checkAsyncTaskStatusAndUpdateUi()};a.prototype.startPolling=function(){this.pollingIntervalId=setInterval(this.checkStatus.bind(this),this.settings.data.pollingInterval)};a.prototype.stopPolling=function(){clearInterval(this.pollingIntervalId)};
a.prototype.deleteExistingZiptaskAnswers=function(){CCL.CommonFunctions.safeAjax({url:this.settings.urls.deleteExistingZipAnswerstask,type:"DELETE",dataType:"json"})};a.prototype.checkBeforeDeleteAnswers=function(a){var b=this;0===window[this.settings.controls.gridId].getSelectedValues().length?CCL.CommonFunctions.jAjaxAlert(this.settings.terms.selectMessage,"",this.settings.terms.ok):CCL.CommonFunctions.jAjaxConfirm(this.settings.terms.deleteAnswersQuestion,"",this.settings.terms.deleteButton,this.settings.terms.cancel,
function(a){a&&(new Function(b.settings.controls.deleteAnswersPostbackRef))()});return!1};a.prototype.checkBeforeAssess=function(){return 0===window[this.settings.controls.gridId].getSelectedValues().length?(CCL.CommonFunctions.jAjaxAlert(this.settings.terms.selectMessage,"",this.settings.terms.ok),!1):!0};a.prototype.checkBeforeDownload=function(a){var b=this;return this.checkBeforeActionOnAnswers(a,function(a){a?(new Function(b.settings.controls.downloadAnswersPostbackRef))():CCL.CommonFunctions.jAjaxAlert(b.settings.terms.nothingToDownload,
"",b.settings.terms.ok)})};a.prototype.checkBeforeActionOnAnswers=function(a,c){var b=window[this.settings.controls.gridId].getSelectedValues(),d=$(a.currentTarget);if(d.hasClass("disabled"))return!1;if(0===b.length)return CCL.CommonFunctions.jAjaxAlert(this.settings.terms.selectMessage,"",this.settings.terms.ok),!1;d.addClass("disabled");this.checkIfAnyAnswersExist(b,d,function(a){c(a);d.removeClass("disabled");d.parent("li").trigger("click")});return!1};a.prototype.checkIfAnyAnswersExist=function(a,
c,d){var b=this;$.ajax({url:this.settings.urls.isAnyAnswerSubmittedByUsers,contentType:"application/json",dataType:"json",type:"POST",data:JSON.stringify({assignmentId:this.settings.data.assignmentId,assignmentInstanceId:this.settings.data.assignmentInstanceId,userIds:a})}).done(function(a){d(a)}).fail(function(){c.removeClass("disabled");CCL.CommonFunctions.jAjaxAlert(b.settings.terms.errorOccured,"",b.settings.terms.ok)})};a.assessLinkDataAttribute="data-for-grader";return a}(c.Base);c.Read=e})(Assignment||
(Assignment={}));
